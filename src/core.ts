...
export function createActaro(options: ActaroOptions = {}) {
  ...
  const inProgressRuns = new Map<string, Promise<ActionReceipt>>();
  ...
  return {
    ...
    async run<S extends z.ZodTypeAny, E>(
      ...
      const idempotencyKey = action.idempotencyKey ? action.idempotencyKey(input) : undefined;

      if (idempotencyKey) {
        const inProgress = inProgressRuns.get(idempotencyKey);
        if (inProgress) return inProgress;
      }

      const promise = (async () => {
        if (idempotencyKey && store.findByIdempotencyKey) {
      ...
      if (idempotencyKey) {
        inProgressRuns.set(idempotencyKey, promise);
        // The finally block must be attached to the promise itself to ensure
        // cleanup happens whenever the promise settles, without blocking execution here.
        promise.finally(() => inProgressRuns.delete(idempotencyKey));
      }
...
export function toAgentResult(receipt: ActionReceipt): {
  ...
  const canClaimCompletion = receipt.status === "verified";
  let message = `Actaro Validation Result: ${receipt.status.toUpperCase()}.`;

  if (canClaimCompletion) {
    message += ` Evidence: ${JSON.stringify(receipt.evidence)}`;
  } else {
    if (receipt.reason) message += ` Reason: ${receipt.reason}.`;
    if (receipt.execution?.error) message += ` Execution Error: ${receipt.execution.error}.`;
  }

  return { message, canClaimCompletion };
}
